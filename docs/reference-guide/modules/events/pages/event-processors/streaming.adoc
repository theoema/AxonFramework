...
====
For multi-tenant applications, see the xref:multitenancy-guide::event-processors.adoc[Multi-Tenancy event processors guide].
The multi-tenancy extension creates per-tenant `PooledStreamingEventProcessor` instances, each with its own event source, token store, and thread pools.
====
