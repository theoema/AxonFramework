 ...
 * {@link TenantRoutingQueryBus}, ensuring interceptors are invoked before
 ...
 * <pre>
 *     InterceptingQueryBus → TenantRoutingQueryBus → TenantSegments
 * </pre>
 *
 * @author Stefan Dragisic
 * @since 5.0.0
 */
class InterceptingTenantRoutingQueryBusTest {
    ...
    private TenantRoutingQueryBus multiTenantQueryBus;
    ...
    void setUp() {
        ...
        // Create TenantRoutingQueryBus that routes to tenant2
        multiTenantQueryBus = new TenantRoutingQueryBus(
                tenant -> {
    ...
    class DispatchInterceptors {
        ...
        void dispatchInterceptorsAreInvokedBeforeTenantRouting() {
            ...
            // then
            assertThat(interceptorInvoked.get()).as("Dispatch interceptor should be invoked").isTrue();
        ...
        void dispatchInterceptorsCanModifyQueryBeforeRouting() {
            ...
            // then - verify the tenant segment received the modified query
            ...
            assertThat(dispatchedQuery.metadata().containsKey("intercepted"))
                    .as("Query should contain interceptor-added metadata")
                    .isTrue();
            assertThat(dispatchedQuery.metadata().get("intercepted")).isEqualTo("true");
        }
        ...
        void multipleDispatchInterceptorsAreInvokedInOrder() {
            ...
            QueryMessage dispatchedQuery = queryCaptor.getValue();
            assertThat(dispatchedQuery.metadata().containsKey("first"))
                    .as("First interceptor metadata should be present")
                    .isTrue();
            assertThat(dispatchedQuery.metadata().containsKey("second"))
                    .as("Second interceptor metadata should be present")
                    .isTrue();
        }
        ...
        void interceptorCanShortCircuitQuery() {
            ...
            // then
            assertThat(result.first().asCompletableFuture().isCompletedExceptionally())
                    .as("Result should be completed exceptionally when interceptor short-circuits")
                    .isTrue();
    ...
    class TenantRouting {
        ...
        void queryIsRoutedToCorrectTenantAfterInterception() {
            ...
            // then - tenant resolver is configured to route to TENANT_2
            verify(tenantSegment2).query(any(), any());
            verify(tenantSegment1, never()).query(any(), any());
        }
        ...
        void interceptedQueryReachesCorrectTenantSegment() {
            ...
            // then
            assertThat(result.first().asCompletableFuture().isCompletedExceptionally()).isFalse();

            ArgumentCaptor<QueryMessage> queryCaptor = ArgumentCaptor.forClass(QueryMessage.class);
            verify(tenantSegment2).query(queryCaptor.capture(), any());

            QueryMessage receivedQuery = queryCaptor.getValue();
            assertThat(receivedQuery.metadata().get("tenant-context"))
                    .as("Tenant segment should receive enriched query")
                    .isEqualTo("enriched");
        }
    }
    ...
    class ResultHandling {
        ...
        void resultFromTenantSegmentIsReturnedThroughInterceptorChain() {
            ...
            MessageDispatchInterceptor<Message> resultTrackingInterceptor =
                    new MessageDispatchInterceptor<>() {
                        ...
                        public MessageStream<?> interceptOnDispatch(Message message,
                                                                    ...
                                                                    MessageDispatchInterceptorChain<Message> chain) {
                            return chain.proceed(message, context)
            ...
            // then
            assertThat(resultIntercepted.get())
                    .as("Result should pass back through interceptor chain")
                    .isTrue();
        }
        ...
        void interceptorCanModifyResult() {
            ...
            assertThat(responseMessage.metadata().get("result-enriched"))
                    .as("Result should contain interceptor-added metadata")
                    .isEqualTo("yes");
        }
    }
    ...
    class SubscriptionQueries {
        ...
        void subscriptionQueryInterceptorsAreInvokedBeforeTenantRouting() {
            ...
            // then
            assertThat(interceptorInvoked.get())
                    .as("Dispatch interceptor should be invoked for subscription query")
                    .isTrue();
        ...
        void subscriptionQueryModifiedByInterceptorReachesCorrectTenant() {
            ...
            QueryMessage dispatchedQuery = queryCaptor.getValue();
            assertThat(dispatchedQuery.metadata().get("subscription-context"))
                    .as("Subscription query should contain interceptor-added metadata")
                    .isEqualTo("enriched");
        }
    }
}
