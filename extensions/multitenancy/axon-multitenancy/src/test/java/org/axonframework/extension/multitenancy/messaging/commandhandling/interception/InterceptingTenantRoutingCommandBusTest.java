 ...
 * {@link TenantRoutingCommandBus}, ensuring interceptors are invoked before
 ...
 * <pre>
 *     InterceptingCommandBus → TenantRoutingCommandBus → TenantSegments
 * </pre>
 *
 * @author Stefan Dragisic
 * @since 5.0.0
 */
class InterceptingTenantRoutingCommandBusTest {
    ...
    private TenantRoutingCommandBus multiTenantCommandBus;
    ...
    void setUp() {
        ...
        // Create TenantRoutingCommandBus that routes to tenant2
        multiTenantCommandBus = new TenantRoutingCommandBus(
                tenant -> {
    ...
    class DispatchInterceptors {
        ...
        void dispatchInterceptorsCanModifyCommandBeforeRouting() {
            ...
            // then - verify the tenant segment received the modified command
            ...
            assertThat(dispatchedCommand.metadata().containsKey("intercepted"))
                    .as("Command should contain interceptor-added metadata")
                    .isTrue();
            assertThat(dispatchedCommand.metadata().get("intercepted")).isEqualTo("true");
        }
        ...
        void multipleDispatchInterceptorsAreInvokedInOrder() {
            ...
            CommandMessage dispatchedCommand = commandCaptor.getValue();
            assertThat(dispatchedCommand.metadata().containsKey("first"))
                    .as("First interceptor metadata should be present")
                    .isTrue();
            assertThat(dispatchedCommand.metadata().containsKey("second"))
    ...
    class TenantRouting {
        ...
        void interceptedCommandReachesCorrectTenantSegment() {
            ...
            assertThat(receivedCommand.metadata().get("tenant-context"))
                    .as("Tenant segment should receive enriched command")
                    .isEqualTo("enriched");
        }
    }
    ...
    class ResultHandling {
        ...
        void interceptorCanModifyResult() {
            ...
            assertThat(resultMessage.metadata().get("result-enriched"))
