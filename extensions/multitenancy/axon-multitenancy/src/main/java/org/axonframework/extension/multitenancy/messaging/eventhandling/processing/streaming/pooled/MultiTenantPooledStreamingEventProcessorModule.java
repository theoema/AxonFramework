 ...
 * with a dedicated event source (from {@link MultiTenantEventStore#tenantSegments()}), token store (from
 ...
 * {@link org.axonframework.extension.multitenancy.messaging.eventhandling.processing.MultiTenantEventProcessorModule#pooledStreaming(String)}.
 * <p>
 * Example usage:
 * <pre>{@code
...
public class MultiTenantPooledStreamingEventProcessorModule
    ...
    private PooledStreamingEventProcessor createTenantProcessor(
        ...
        // Per-tenant event source from the MultiTenantEventStore
        parent.getOptionalComponent(EventStore.class)
              .filter(MultiTenantEventStore.class::isInstance)
              .map(MultiTenantEventStore.class::cast)
              .map(MultiTenantEventStore::tenantSegments)
