 ...
 * <p>
 * Follows the same pattern as {@link TenantRoutingEventStore}:
 * the tenant is resolved from the current message's metadata via the {@link TenantResolver},
 * and the operation is delegated to the corresponding tenant's {@link SnapshotStore} segment.
 *
 * @author Theo Emanuelsson
 * @since 5.2.0
 */
public class TenantRoutingSnapshotStore implements SnapshotStore, MultiTenantAwareComponent, DescribableComponent {

    /**
     * The order in which the {@link TenantRoutingSnapshotStore} is applied as a decorator to the {@link SnapshotStore}.
     */
    ...
    /**
     * Instantiate a {@link TenantRoutingSnapshotStore} with the given {@code tenantSegmentFactory} and
     * {@code tenantResolver}.
     *
     ...
     */
    public TenantRoutingSnapshotStore(TenantSnapshotStoreSegmentFactory tenantSegmentFactory,
    ...
    private SnapshotStore resolveTenantStore(@Nullable ProcessingContext context) {
        ...
        TenantDescriptor tenant = TenantResolver.fromContext(context, tenantResolver, tenantSegments.keySet());
